...
export class AuthService {
  ...
  async exchangeCodeForToken(
    code: string,
    redirectUri: string,
  ): Promise<MondayAccessToken> {
    this.logger.info('Starting exchanging code for token');
    const cliendtId = this.manageService.getEnv('MDY_CLIENT_ID');
    const clientSecret = this.manageService.getSecret('MDY_CLIENT_SECRET');
    ...
    try {
      ...
      this.logger.error(
        `Failed to exchange code for token with error: ${errorMessage}`,
      );
      const errorResponse = StandardResponse.error(
        null,
  ...
  async storeAccessToken(accountId: string, token: MondayAccessToken) {
    ...
    try {
      await secureStorage.set<MondayAccessToken>(
        `${ACCESS_TOKEN_PREFIX}${accountId}`,
        token,
      );
      ...
      this.logger.info(
        `Access token stored successfully for accountId: ${accountId}`,
      );
    } catch (error) {
      const errorMessage =
        error instanceof Error ? error.message : String(error);
      this.logger.error(
        `Failed to store access token for accountId: ${accountId} with error: ${errorMessage}`,
      );
  ...
  async getAccessToken(accountId: string): Promise<MondayAccessToken | null> {
    ...
    if (cacheToken) {
      this.logger.info(
        `Access token retrieved from cache for accountId: ${accountId}`,
      );
      return cacheToken;
    }
    try {
      const secureStore = this.manageService.getSecureStorage();
      const token = await secureStore.get<MondayAccessToken>(
        `${ACCESS_TOKEN_PREFIX}${accountId}`,
      );
      if (token) {
        this.tokenCacheService.set(accountId, token);
        this.logger.info(
          `Access token retrieved from secure storage for accountId: ${accountId}`,
        );
        return token;
      }
      return null;
      ...
      const errorMessage =
        error instanceof Error ? error.message : String(error);
      this.logger.error(
        `Failed to retrieve access token for accountId: ${accountId} with error: ${errorMessage}`,
      );
      const errorResponse = StandardResponse.error(
        null,
  ...
  async removeAccessToken(accountId: string) {
    ...
    try {
      ...
      this.logger.info(
        `Access token removed successfully for accountId: ${accountId}`,
      );
    } catch (error) {
      const errorMessage =
        error instanceof Error ? error.message : String(error);
      this.logger.error(
        `Failed to remove access token for accountId: ${accountId} with error: ${errorMessage}`,
      );
      const errorResponse = StandardResponse.error(
        null,
  ...
  createJwtToken(
    type: 'MDY_SIGNING_SECRET' | 'MDY_CLIENT_SECRET',
    ...
    try {
      ...
      if (!secret || typeof secret !== 'string') {
        this.logger.error(
          `Signing secret not found or invalid for type: ${type}`,
        );
        const errorResponse = StandardResponse.error(
          null,
      ...
      const errorMessage =
        error instanceof Error ? error.message : String(error);
      this.logger.error(
        `Failed to create JWT token for type: ${type} with error: ${errorMessage}`,
      );
      const errorResponse = StandardResponse.error(
        null,
